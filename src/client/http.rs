...
pub fn update(game_id: &str, player_id: &str, position: (f32, f32)) -> Result<String, Box<dyn std::error::Error>> {
    let client = reqwest::blocking::Client::new();
    match client.post("http://localhost:1337/update")
        .body(serde_json::json!({ "game_id": game_id, "player_id": player_id, "position": [position.0, position.1] }).to_string())
        .send()?
        .text() {
